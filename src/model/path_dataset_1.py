...
class Dataset(object):
    ...
    def __next__(self):
        ...
        self.ptr += 1
        # 为sample设置一些值
        sample = {'support_set': [], 'sent_doc_mp': np.array(0), 'doc_label': np.array(0)}
        ...
        # 保证在正样本不为0的时候，负样本 = 正样本；当正样本=0的时候，负样本为全集。
        all_neg = doc['neg_ins'][:self.max_entity_pair_num - len(doc['pos_ins'])]
        all_neg_half = all_neg[::2]
        random.shuffle(all_neg)
        random.shuffle(all_neg_half)
        if self.mode == 'train':
            if len(doc['pos_ins']) != 0:
                all_ins = doc['pos_ins'] + all_neg[:round(1 * len(all_neg))]
            else:
                all_ins = all_neg[:round(1 * len(all_neg))]
        elif self.mode == 'dev':
            all_ins = doc['pos_ins'] + all_neg
        ...
        for ins_idx, ins in enumerate(all_ins):
            ...
            # 负样本对半处理
            # 正样本相反的负样本
            if ins_idx < len(doc['pos_ins']):
                if [ins['head'], ins['tail']] in posToNeg_head_tail:
                    posToNeg_head_tail.remove([ins['head'], ins['tail']])
                else:
                    posToNeg_head_tail.append([ins['tail'], ins['head']])
            # 保留正样本
                half_all_head_tail.append([ins['head'], ins['tail']])
                half_all_ins.append(ins)
                half_all_label.append(ins['label'])
                sup1.append(ins['support_set'])
                head_mask1.append(ins['head_mask'])
                tail_mask1.append(ins['tail_mask'])
            #  保留一半的负样本
            else:
                if [ins['head'], ins['tail']] not in posToNeg_head_tail:
                    # 对半的负样本
                    if [ins['tail'], ins['head']] not in other_half_head_tail:
                        other_half_head_tail.append([ins['head'], ins['tail']])
                        other_half_sample.append(ins)
                        other_half_label.append(ins['label'])
                        sup3.append(ins['support_set'])
                        head_mask3.append(ins['head_mask'])
                        tail_mask3.append(ins['tail_mask'])
                else:
                    # 保留与正样本头尾实体互换的负样本
                    posToNeg_sample.append(ins)
                    posToNeg_label.append(ins['label'])
                    sup2.append(ins['support_set'])
                    head_mask2.append(ins['head_mask'])
                    tail_mask2.append(ins['tail_mask'])
        ...
        sample['all_sample'] = half_all_head_tail + posToNeg_head_tail + other_half_head_tail
        sample['label'] = half_all_label + posToNeg_label + other_half_label
        sample['posToNeg_head_tail'] = len(posToNeg_head_tail)
        sample['other_half_head_tail'] = len(other_half_head_tail)
        sample['num_pos_sample'] = len(doc['pos_ins'])
        sample['support_set'] = sup1 + sup2 + sup3
        sample['head_mask'] = head_mask1 + head_mask2 + head_mask3
        sample['tail_mask'] = tail_mask1 + tail_mask2 + tail_mask3
        for key in ['head_mask', 'tail_mask']:
        ...
        sample['doc_label'] = torch.tensor(doc['doc_label'], dtype=torch.float)
        for fl in ['mention_all_mask', "entity_all_mask", "sentences_mask"]:
            sample[fl] = torch.tensor(sample[fl], dtype=torch.float)
        for i in ["M_M", "M_E", "M_S", "E_S", "S_S", 'all_sample', 'support_set']:
